...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(jwtInterceptor)
                ...
                .addPathPatterns("/api/order/**", "/api/cart/**", "/api/merchant/**", "/api/chat/**", "/api/live/**", "/api/user/profile", "/api/user/address/**", "/api/product/favorite/**", "/api/product/favorites", "/api/coupon/**")
                // 同时，告诉保安不要去管登录、注册和查看商品列表的接口
                .excludePathPatterns("/api/user/login", "/api/user/register", "/api/product/list", "/api/product/paged", "/api/product/detail/**", "/api/product/search", "/api/product/reviews/**", "/api/category/**");
    }
