...
public class OrderTask {
    public final StringRedisTemplate stringstringRedisTemplate;
    ...
    public void processDelayedOrders() {
        ...
        Set<String> timeoutOrderId = stringstringRedisTemplate.opsForZSet().rangeByScore("order:timeout:queue", 0, now);
        if (timeoutOrderId != null && !timeoutOrderId.isEmpty()) {
            for (String orderIdStr : timeoutOrderId) {
                ...
                Long removed = stringstringRedisTemplate.opsForZSet().remove("order:timeout:queue", orderIdStr);
